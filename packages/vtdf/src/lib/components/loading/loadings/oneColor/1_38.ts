	...
	setup(props: LoadingAnimationProps) {
		return () => {
			...
			return h(Fragment, null, [
				...
				h('div', { class: 'rtdf_loading_1_38' }, [
					h('div', { class: resolveLoadingLayoutClass({ kind: 'center', size }) }, [
						h(
							'svg',
							{
								class: resolveLoadingLayoutClass({ kind: 'svgFull' }),
								x: '0px',
								y: '0px',
								viewBox: loadingOneColor38Svg.viewBox,
								height: loadingOneColor38Svg.height,
								width: loadingOneColor38Svg.width,
								preserveAspectRatio: 'xMidYMid meet'
							},
							[
								h('path', {
									class: resolveLoadingSvgStrokeClass({
										className: 'track opacity-10',
										strokeClass: loadingClassState.strokeClass
									}),
									style: parseStyle(resolveLoadingOneColorStrokeStyle({ customColor })),
									'stroke-width': loadingOneColor38Svg.path.strokeWidth,
									fill: 'none',
