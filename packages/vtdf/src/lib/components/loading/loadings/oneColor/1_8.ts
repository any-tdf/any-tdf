	...
	setup(props: LoadingAnimationProps) {
		return () => {
			...
			return h(Fragment, null, [
				...
				h('div', { class: 'rtdf_loading_1_8' }, [
					h(
						'div',
						{
							class: resolveLoadingLayoutClass({ kind: 'loadingFlexBetween', size }),
							style: parseStyle(resolveLoadingBaseAnimationStyle({ speed }))
						},
						[
							h('div', {
								class: resolveLoadingRoundDotClass({ bgClass: loadingClassState.bgClass }),
								style: parseStyle(resolveLoadingOneColorBackgroundStyle({ customColor }))
							}),
							h('div', {
								class: resolveLoadingRoundDotClass({ bgClass: loadingClassState.bgClass }),
								style: parseStyle(resolveLoadingOneColorBackgroundStyle({ customColor }))
