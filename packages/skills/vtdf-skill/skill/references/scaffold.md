...
Use this reference when creating or modifying VTDF scaffold behavior.
...
```sh
bun create any-tdf@latest my-app -f vue -t vrtt -l zh_CN -i iconify -m multi -b lucide
```
...
| `-f`, `--framework`             | required in non-interactive mode | Framework, use `vue` for VTDF                 |
...
| ------ | ------------------------------------ | ------ |
| `vrtt` | Vite + Vue 3 + Tailwind + TypeScript | Ready  |
| `vrut` | Vite + Vue 3 + UnoCSS + TypeScript   | Ready  |
...
- CLI source: `create/src/index.js`.
- Prompt locale source: `create/src/lang.js`.
- Templates: `create/templates/vue`.
- Icon snippets: `create/snippet/vue`.
