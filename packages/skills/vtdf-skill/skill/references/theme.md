...
```ts
import { getMode, switchMode } from 'vtdf/theme';
...
VTDF themes are CSS variables scoped by `[data-theme="<name>"]`. Use the Tailwind CSS 4 plugin from VTDF in entry CSS:
...
```ts
import { getTheme, switchTheme } from 'vtdf/theme';
...
In custom themes, `color-primary` and `color-dark` are base colors. The VTDF theme plugin calculates 50, 100, 200, 300, 400, 500, default 600, 700, 800, 900, and 950.
