...
Use this reference when designing or reviewing VTDF colors, including content from the documentation site's non-Markdown color page.

## Color Groups

VTDF color system has four groups:
...
Theme colors are the most frequently used brand or accent colors. VTDF intentionally separates light-mode and dark-mode theme colors:

- `primary` is optimized for light surfaces.
- `dark` is optimized for dark surfaces.

VTDF recommends using different hues for light and dark mode when it improves visual clarity. Matching hues are allowed if the product needs strict brand consistency.

## Gradient Palette

VTDF follows Tailwind color naming:
