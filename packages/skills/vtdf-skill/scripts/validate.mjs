...
const vtdfRoot = path.resolve(packageRoot, '../..');
const anyRoot = path.resolve(vtdfRoot, '..');
...
if (existsSync(skillPath)) {
	...
	if (!skill.startsWith('---\nname: vtdf\n')) {
		failures.push('SKILL.md frontmatter must start with name: vtdf');
	}
	if (!/description: .+VTDF/.test(skill)) {
		failures.push('SKILL.md frontmatter must include a VTDF description');
	}
}
...
if (existsSync(agentPath)) {
	...
	if (!agent.includes('interface:') || !agent.includes('default_prompt:') || !agent.includes('$vtdf')) {
		failures.push('skill/agents/openai.yaml must define interface.default_prompt mentioning $vtdf');
	}
}
...
[skillPath, ...walkFiles(path.join(packageRoot, 'skill/references')).filter((file) => file.endsWith('.md'))].forEach((file) => {
	...
	if (/docs\/mds|packages\/vtdf\/src|_Source:|Repository Pointers|Route Examples|Source Files/.test(content)) {
...
console.log('vtdf-skill validation passed.');
