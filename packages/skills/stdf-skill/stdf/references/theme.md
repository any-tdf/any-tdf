...
STDF dark mode is driven by `data-mode` on the `html` element. Configure Tailwind CSS 4 with:
...
```ts
import { getMode, switchMode } from 'stdf/theme';
...
STDF themes are CSS variables scoped by `[data-theme="<name>"]`. Use the Tailwind CSS 4 plugin from `@any-tdf/common` in entry CSS:
...
```ts
import { getTheme, switchTheme } from 'stdf/theme';
...
In custom themes, `color-primary` and `color-dark` are base colors. The STDF theme plugin calculates 50, 100, 200, 300, 400, 500, default 600, 700, 800, 900, and 950.
...
When using `@plugin "stdf/theme"`, still define a default `@theme` block. Tailwind needs default variables to generate utilities such as `bg-primary` and `text-dark`. The plugin then overrides those variables under `[data-theme="<name>"]`.

Use `scripts/generate-theme.mjs --preset ANYTDF --format both` from the skill root to generate both the plugin block and the default `@theme` block. `STDF` remains a legacy preset alias for compatibility, but new output uses the current `ANYTDF` theme name.
