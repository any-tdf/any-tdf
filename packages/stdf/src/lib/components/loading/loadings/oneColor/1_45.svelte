<script lang="ts">
	import { resolveLoadingLayoutClass, loadingFourDotIndexes, resolveLoadingPairedPendulumBarClass, resolveLoadingPairedPendulumCss, resolveLoadingPairedPendulumDotClass, resolveLoadingPairedPendulumDotStyle, resolveLoadingOneColorClassState, resolveLoadingOneColorBackgroundColorStyle } from '@any-tdf/common/derived/loading';
	type Props = { theme?: boolean; inverse?: boolean; size?: string; customColor?: any[]; speed?: number };
	let { theme = false, inverse = false, size = 'w-8 h-8', customColor = [], speed = $bindable(1) }: Props = $props();

	const loadingClassState = $derived(resolveLoadingOneColorClassState({ theme, inverse }));
	const css = resolveLoadingPairedPendulumCss({ scope: 'stdf_loading_1_45' });
</script>

<svelte:head>
	{@html `<style>${css}</style>`}
</svelte:head>

<div class="stdf_loading_1_45">
	<div class={resolveLoadingLayoutClass({ kind: 'flexCenterJustified', size })}>
		{#each loadingFourDotIndexes as item (item)}
			<div
				class={resolveLoadingPairedPendulumDotClass(item)}
				style={resolveLoadingPairedPendulumDotStyle(speed)}
			>
				<div class={resolveLoadingPairedPendulumBarClass(loadingClassState.bgClass)} style={resolveLoadingOneColorBackgroundColorStyle({ customColor })}></div>
			</div>
		{/each}
	</div>
</div>
