<script lang="ts">
	...
	const css = resolveLoadingLineRotateCss({ scope: 'stdf_loading_1_53' });
</script>
...
<div class="stdf_loading_1_53">
	<div class={resolveLoadingLayoutClass({ kind: 'relativeCenter', size })}>
		{#each loadingRotatingLineFrames as item, i (i)}
			<div
				class={resolveLoadingRotatingLineClass(loadingClassState.bgClass)}
				style={resolveLoadingRotatingLineStyle({ color: customColor[0], frame: item, speed, top: '50%' })}
			></div>
		{/each}
	</div>
</div>
