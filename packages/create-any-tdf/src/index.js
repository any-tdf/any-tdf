...
if (argv.help || argv.h) {
	console.log(`create-any-tdf v${version}

Usage:
  create-any-tdf [project-name] [options]

Options:
  -f, --framework <name>            Framework: svelte (stdf), react (rtdf), or vue (vtdf)
  -t, --template <name>             Template, svelte: sktt | skut | vstt | vsut, react/vue: vrtt | vrut
                                    (default: first template of the selected framework)
  -l, --language <lang>             Prompt language: ${Object.keys(langAll).join(', ')} (default: en_US)
  -i, --icon-usage <mode>           Icon usage: svg-symbol | iconify | both | none (default: svg-symbol)
  -m, --theme-mode <mode>           Theme mode: single | multi | all (default: multi)
  -b, --built-in-icon-library <id>  Built-in icon library: default | remix | lucide | phosphor | tabler | iconoir | reicon (default: default)
  -p, --package-manager <pm>        Package manager: bun | npm | pnpm | yarn
                                    (default: inferred from npm_config_user_agent, fallback bun)
      --local-packages [dir]        Use local workspace packages instead of registry versions
  -h, --help                        Show this help
  -v, --version                     Show version

Examples:
  create-any-tdf
  create-any-tdf my-app -f svelte -t sktt -b lucide
  create-any-tdf my-app -f react -t vrtt -l zh_CN -i both -m all -p pnpm`);
	process.exit(0);
}
...
// Look up the latest CLI version in the background so an unreachable registry
// cannot block the first screen; the result is read after project generation.
const createAnyTdfVersionPromise = getLatestVersion('create-any-tdf').catch(() => null);
...
// Infer the invoking package manager from npm_config_user_agent (e.g. "pnpm/10.0.0 ..."), fallback to bun.
const detectPackageManager = () => {
	const userAgentName = (process.env.npm_config_user_agent || '').split(' ')[0].split('/')[0];
	return packageManagerOptions.find((item) => item.value === userAgentName) || packageManagerOptions[0];
};
...
const addLocalFrameworkRuntimeDependencies = (packageJson, frameworkItem) => {
	...
	const runtimeDependencyMap = {
		react: ['@any-tdf/common', '@any-tdf/react-confetti', '@any-tdf/react-motion'],
		svelte: ['@any-tdf/common'],
		vue: ['@any-tdf/common', '@any-tdf/vue-confetti', '@any-tdf/vue-motion']
	};
	...
	for (const packageName of runtimeDependencyMap[frameworkItem.value]) {
		...
		if (packageName.endsWith('-confetti')) {
...
const exitInvalid = (message, options) => {
	const suffix = options ? ' (' + getOptionValues(options).join(', ') + ')' : '';
	p.log.error(red(message + suffix));
	process.exit(1);
};
...
const getIconMarkerReplacement = (frameworkItem, snippet) => {
	...
	return `${formatBlockForMarker(snippet, indent)}\n\n${indent}${marker}`;
};
...
const createFunc = async (
	...
	const createAnyTdfVersion = await Promise.race([
		createAnyTdfVersionPromise,
		new Promise((resolve) => setTimeout(() => resolve(null), 3000))
	]);
	if (createAnyTdfVersion && version !== createAnyTdfVersion) {
		console.log(red(`Recommended to use the latest version: ${createAnyTdfVersion}`));
	}
	process.exit(0);
};
...
// Simple npm package name validation (core rules of validate-npm-package-name):
// only lowercase letters, digits, hyphens, underscores, and dots;
// must not start with a dot or an underscore; no spaces or uppercase letters.
const getProjectNameError = (projectName) => {
	const packageName = path.basename(path.resolve(String(projectName)));
	if (packageName.length > 214 || !/^[a-z0-9._-]+$/.test(packageName) || /^[._]/.test(packageName)) {
		return lang.pniv;
	}
	return null;
};
...
const selectProjectName = async (frameworkItem) => {
	if (argvProjectName) {
		const argProjectNameError = getProjectNameError(argvProjectName);
		if (argProjectNameError) exitInvalid(argProjectNameError);
		return argvProjectName;
	}
	...
	const projectName = await p.text({
		...
		validate: (value) => {
			...
			const projectNameError = getProjectNameError(value);
...
const selectPackageManager = async (itemPackageManager) => {
	...
	if (argvProjectName) return detectPackageManager();

	const packageManager = await p.select({
		message: bold(lang.pm),
		options: packageManagerOptions,
		initialValue: detectPackageManager().value
	});
...
if (packageManager.value === 'yarn') {
	p.log.warn(
		'The generated project uses npm: dependency aliases, which Yarn classic (1.x) does not support. Please use Yarn Berry (>= 2), or pick npm, pnpm, or bun.'
	);
}
