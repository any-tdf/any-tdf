...
| left       | `'back'\|null`\|[`Icon`](https://stdf.dev/components?nav=icon&tab=1) | `'back'`                | N    | 左侧内容，`'back'` 返回图标，`null` 无内容，否则渲染图标。 |
| rights     | [`Icon[]`](https://stdf.dev/components?nav=icon&tab=1)               | `[]`                    | N    | 右侧内容，Icon Props 组成的数组，建议不超过三个。          |
