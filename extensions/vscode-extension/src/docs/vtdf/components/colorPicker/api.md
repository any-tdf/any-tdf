...
| 名称   | 类型                         | 参数                                  | 描述                             |
| ------ | ---------------------------- | ------------------------------------- | -------------------------------- |
| change | `(colors: string[]) => void` | colors - 当前颜色值数组（各模式格式） | 颜色变化时触发（直接显示模式）。 |
| close  | `(colors: string[]) => void` | colors - 当前颜色值数组（各模式格式） | 关闭时触发（Popup 模式）。       |
| copy   | `(text: string) => void`     | text - 复制的文本                     | 复制颜色值时触发。               |
...
`change` 和 `close` 事件返回的 colors 数组包含当前颜色在各模式下的字符串表示：
