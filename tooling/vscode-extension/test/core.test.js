...
describe('package detection', () => {
	...
	test('continues upward when the nearest package has no target dependency', async () => {
		...
		writeJson(path.join(root, 'workspace/package.json'), { devDependencies: { rtdf: '3.0.0-alpha.0' } });
		...
		expect(info.currentVersion).toBe('3.0.0-alpha.0');
	});
...
describe('extension manifest', () => {
	test('declares external Svelte and Vue language extensions', () => {
		...
		expect(manifest.extensionPack).toEqual(['svelte.svelte-vscode', 'vue.volar']);
