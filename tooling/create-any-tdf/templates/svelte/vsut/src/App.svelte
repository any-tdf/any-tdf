<script lang="ts">
	...
	const storageGet = (key: string) => (typeof localStorage === 'undefined' ? null : localStorage.getItem(key));
	const sessionStorageGet = (key: string) => (typeof sessionStorage === 'undefined' ? null : sessionStorage.getItem(key));
...
<ConfigProvider {locale} {builtInIconLibrary}>
	<div class="flex items-center justify-center gap-2 pt-10 text-center">
		...
		<a class="flex w-10 flex-col items-center" href="https://unocss.dev" target="_blank" rel="noreferrer">
			<div class="flex h-10 w-10 items-center justify-center rounded-full bg-orange text-sm font-bold text-white">U</div>
		</a>
	...
	<div class="my-6 text-center text-xs">
		{#if lang === 'zh_CN'}
			<p>这是 Vite & Svelte & UnoCSS & TypeScript & STDF 构建的模板</p>
			<p class="mt-2">点击上方 LOGO 了解更多</p>
		{:else}
			<p>This is a template using Vite & Svelte & UnoCSS & TypeScript & STDF</p>
