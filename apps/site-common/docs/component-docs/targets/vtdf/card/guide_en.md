...
The Card component is a versatile container component used to organize content within a card container with rounded corners, shadow, and border. Simply pass the content to be displayed as the default Vue Slot.
...
When the `@click` event is listened to, the card renders as a `button` element with clickable interaction.
