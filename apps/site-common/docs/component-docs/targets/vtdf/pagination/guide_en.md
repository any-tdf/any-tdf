...
In my opinion, pagination should not be based on scrolling up to load the next page. This interaction can cause user uncomfortable as there is no psychological expectation. Therefore, the VTDF pagination component meet the following conditions by default:
...
The Pagination component exposes three events: `@change`, `@next`, and `@pre`, corresponding to page number changes, next page, and previous page. The `@next` and `@pre` events also trigger `@change`, so listen as needed.
