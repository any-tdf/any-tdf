...
When using the keyboard on mobile devices, consider the keyboard height because it may cover areas where input content needs to be displayed. FullKeyboard automatically calculates the keyboard height when it opens and dispatches it through the `@open` event. Developers can use this value to handle keyboard coverage.
