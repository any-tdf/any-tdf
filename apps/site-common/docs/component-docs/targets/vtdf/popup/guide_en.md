...
In the VTDF design philosophy, both BottomSheet and Popup are floating interactions within the interface, generally used for secondary operations within a page. There are some similarities in the use of the components. However, the Popup is a "layer" concept, while the BottomSheet is a "window" concept.
...
By default, clicking the popup mask closes it. If this behavior is not required, set `maskClosable` to `false`. Use `@close` to listen for popup exit, and `@click-mask` to listen for mask clicks. `@click-mask` still triggers when `maskClosable` is `false`.
