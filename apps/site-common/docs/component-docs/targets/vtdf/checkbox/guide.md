...
由于移动端横向显示内容有限，Checkbox 默认为纵向布局，横向布局建议只用作简单使用。当选项较多、内容较长或显示内容较多时建议使用纵向布局。当 Checkbox 内的选项被点击时，通过 `@change` 事件，可以获取到当前所有选定项标识符 `name` 组成的数组。
...
当传入 checkboxChild 这个 Vue Slot 时，选项区域将会以传入的 Vue Slot 内容渲染，可自由设计选中与未选中的效果，获取数据逻辑等。
