...
通过 `children` 传入 Vue Slot 或 render props 可以完全自定义倒计时的显示内容。render props 会接收一个 `TimeData` 对象作为参数，包含 `days`、`hours`、`minutes`、`seconds`、`milliseconds` 属性。

## 事件

- `@finish`：倒计时结束时触发
- `@change`：倒计时变化时触发，参数为当前的 `TimeData` 对象
