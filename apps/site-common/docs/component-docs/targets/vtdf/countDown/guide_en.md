...
Use `children` as Vue Slot or render props to fully customize the countdown display. Render props receive a `TimeData` object with `days`, `hours`, `minutes`, `seconds`, and `milliseconds` properties.

## Events

- `@finish`: Triggered when the countdown ends
- `@change`: Triggered on time change, with the current `TimeData` object as parameter
