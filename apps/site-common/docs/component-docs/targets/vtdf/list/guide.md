...
List 组件用于展示结构化列表内容，通过 `itemChild` Vue Slot 定义每个列表项的渲染内容。
...
- `@batch-change`：选中项变化时触发。
- `@batch-action`：点击批量操作按钮时触发。
...
`SwipeActionProps` 支持 `icon`、`text` 和 `bgColor`，其中 `bgColor` 支持 `theme`、`success`、`warning`、`error`、`info`，`primary` 作为 `theme` 的别名。点击滑动操作按钮时，通过 `@swipe-action` 监听。
...
通过 `headerChild` 和 `footerChild` Vue Slot 可以在列表顶部和底部添加自定义内容。
