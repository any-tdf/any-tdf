...
By listening to the `@next` and `@prev` events, update the current column's new `data` and bind it back to the component. You also need to bind whether the current column is the first level (`lastLevel`) or the last level (`firstLevel`).

When the data is at the last level, the right button triggers `@confirm` and returns the selected data; otherwise it triggers `@next` and returns the index of the current selection.
...
In actual development, AsyncPicker data is usually obtained dynamically, typically through APIs. Generally, we need to process the input data through loops to make it work with the AsyncPicker component. However, this is a cumbersome and unnecessary process. Therefore, VTDF has optimized the AsyncPicker component's input parameters, allowing developers to directly pass in API response data without loop processing.
