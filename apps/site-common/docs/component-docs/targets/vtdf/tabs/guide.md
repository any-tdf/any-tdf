...
Tabs Props 的 transition 默认为 true，此时 Tabs 主体内容切换带有过渡效果。当 transition 为 false，或者 position 为 `l` 或 `r` 时，Tabs 主体内容将不会有过渡效果，此时可以通过 Vue 的 render props 传入内容。children 需要接收当前激活的 Tab 索引值 `{ active }`，再结合 TabContent 的 show 控制主体内容的显示与隐藏。通过暴露出的 active，你还可以自由使用其他方式控制主体区域内容的过渡效果。示例中展示了一种方式，请结合具体业务尝试其他切换效果。
...
Tab 组件可以单独使用，通过绑定的 `@click-tab` 事件，传递出当前激活选项卡的索引值。
