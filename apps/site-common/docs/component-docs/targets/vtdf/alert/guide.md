...
在 VTDF 的反馈体系中，Toast、Alert、Modal、Dialog 都用于向用户传达信息，但侧重点不同。
...
通过 `transitionType` 属性设置过渡动画，支持 VTDF 内置的过渡动画函数：
...
通过 `easeType` 属性设置进入动画的缓动函数，`easeOutType` 设置退出动画的缓动函数，支持 VTDF 内置 easing 列表：
...
通过 `children` Vue Slot 可完全自定义提示内容，实现更复杂的交互效果。
