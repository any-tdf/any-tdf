...
The Mask can be configured for click-through through the `clickable` prop. When it is set to `true`, clicking the mask will not trigger the `@click-mask` event.
