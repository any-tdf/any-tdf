...
| popup              | [`Popup`](https://stdf.dev/components?nav=popup&tab=1)\|`null` | `{}`                                       | N        | Popup parameters. Pass `null` to display without popup.    |
| multiple           | `boolean`                                                      | `false`                                    | N        | Whether to enable multiple selection mode.                 |
| multipleIcon       | [`Icon`](https://stdf.dev/components?nav=icon&tab=1)           | `{ name: 'ri-checkbox-circle-line', ... }` | N        | Icon for unselected state in multiple mode.                |
| multipleIconActive | [`Icon`](https://stdf.dev/components?nav=icon&tab=1)           | `{ name: 'ri-checkbox-circle-fill', ... }` | N        | Icon for selected state in multiple mode.                  |
