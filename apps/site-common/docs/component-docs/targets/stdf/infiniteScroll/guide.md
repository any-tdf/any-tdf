...
组件暴露 `check()` 方法，适合在数据变更、Tab 切换、弹层显示或容器尺寸变化后主动检测当前位置。

## 自定义内容

默认会展示加载中、加载完成和加载失败文案。也可以使用 `loadingChild`、`finishedChild`、`errorChild` snippet 自定义状态内容，snippet 的 `detail` 参数提供 `status` 和 `retry`，其中 `retry` 可直接绑定到自定义重试按钮。传入 `children` 则完全接管状态内容。
