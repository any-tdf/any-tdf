...
| onload | `(isRetry: boolean) => void` | `isRetry` - 是否为失败重试 | 达到触发距离时调用。兼容 STDF 的小写事件别名。 |

## InfiniteScroll Children

| 名称          | 类型                                                             | 参数     | 说明                 |
| ------------- | ---------------------------------------------------------------- | -------- | -------------------- |
| children      | `ReactNode \| ((detail: InfiniteScrollSlotDetail) => ReactNode)` | `detail` | 完全自定义状态内容。 |
| loadingChild  | `ReactNode \| ((detail: InfiniteScrollSlotDetail) => ReactNode)` | `detail` | 加载中内容。         |
| finishedChild | `ReactNode \| ((detail: InfiniteScrollSlotDetail) => ReactNode)` | `detail` | 加载完成内容。       |
| errorChild    | `ReactNode \| ((detail: InfiniteScrollSlotDetail) => ReactNode)` | `detail` | 加载失败内容。       |
