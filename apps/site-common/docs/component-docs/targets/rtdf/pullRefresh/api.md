...
| threshold         | `number`                                                                    | `60`                                                    | N    | 释放触发刷新需要达到的距离，也是阻尼衰减的拐点，单位为像素。 |
| pullFactor        | `number`                                                                    | `1`                                                     | N    | 手势距离到下拉距离的换算系数。                               |
| maxDistance       | `number`                                                                    | `0`                                                     | N    | 最大下拉距离，单位为像素，`0` 表示不限制（仍受阻尼衰减）。   |
| successDuration   | `number`                                                                    | `500`                                                   | N    | 成功状态展示时长，单位为毫秒。                               |
| animationDuration | `number`                                                                    | `300`                                                   | N    | 回弹和状态切换动画时长，单位为毫秒，跟手阶段始终无动画。     |
...
| scrollTarget      | `HTMLElement \| Window \| RefObject<HTMLElement \| null> \| string \| null` | `null`                                                  | N    | 指定滚动容器。                                               |
...
| onrefresh | `() => void`                                | -                       | 释放并达到阈值时触发。兼容 STDF 的小写事件别名。       |
| onChange  | `(detail: PullRefreshChangeDetail) => void` | `detail` - 当前状态信息 | 状态、距离或进度变化时触发。                           |
| onchange  | `(detail: PullRefreshChangeDetail) => void` | `detail` - 当前状态信息 | 状态、距离或进度变化时触发。兼容 STDF 的小写事件别名。 |
...
| normalChild     | `ReactNode \| ((detail: PullRefreshChangeDetail) => ReactNode)` | `detail` | 初始状态内容。   |
| pullingChild    | `ReactNode \| ((detail: PullRefreshChangeDetail) => ReactNode)` | `detail` | 下拉中状态内容。 |
| canReleaseChild | `ReactNode \| ((detail: PullRefreshChangeDetail) => ReactNode)` | `detail` | 可释放状态内容。 |
| refreshingChild | `ReactNode \| ((detail: PullRefreshChangeDetail) => ReactNode)` | `detail` | 刷新中状态内容。 |
| successChild    | `ReactNode \| ((detail: PullRefreshChangeDetail) => ReactNode)` | `detail` | 成功状态内容。   |
