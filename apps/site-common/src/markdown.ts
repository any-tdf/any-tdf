...
export type HighlightLanguageOptions = {
	language: string;
	ignoreIllegals?: boolean;
};

export type HighlightLike = {
	highlight: (code: string, options: HighlightLanguageOptions) => HighlightResult;
	highlightAuto: (code: string) => HighlightResult;
};

export type CodeGroupSvgItem = {
	name: string;
	cli: string;
	svg: string;
};

export type GroupIconMdPluginOptions = {
	codeGroupSvgData?: CodeGroupSvgItem[];
};
...
const htmlEntityRegExp = new RegExp(`(${Object.keys(htmlEntityMap).join('|')})`, 'g');

const htmlFilter = (text: string) => {
	return text.replace(htmlEntityRegExp, (match) => htmlEntityMap[match as keyof typeof htmlEntityMap]);
};
...
export const createMdTextToHljs = (hljs: HighlightLike) => {
	return (mdText: string, language: string = '') => {
		mdText = mdText.replace(/<td><code>/g, `<td><code class="${language ? `language-${language}` : ''}">`);
		if (mdText.indexOf('<code class="') <= 0) {
			return mdText;
		}

		let apiText = htmlFilter(mdText).replace(/<code class="/g, '<code class="hljs ');
		...
		for (let t = 0; t < codeTexts.length; t++) {
			...
			const highlightedCode = currentLanguage
				? hljs.highlight(code, {
...
const pnpmSvg =
	'<svg xmlns="http://www.w3.org/2000/svg" width="1em" height="1em" viewBox="0 0 32 32"><path fill="#f9ad00" d="M30 10.75h-8.749V2H30Zm-9.626 0h-8.75V2h8.75Zm-9.625 0H2V2h8.749ZM30 20.375h-8.749v-8.75H30Z"/><path fill="#4e4e4e" d="M20.374 20.375h-8.75v-8.75h8.75Zm0 9.625h-8.75v-8.75h8.75ZM30 30h-8.749v-8.75H30Zm-19.251 0H2v-8.75h8.749Z"/></svg>';
const npmSvg =
...
export const createCodeGroupSvgData = (packageName: string): CodeGroupSvgItem[] => [
	{ name: 'pnpm', cli: `pnpm create ${packageName}@latest`, svg: pnpmSvg },
	{ name: 'npm', cli: `npm create ${packageName}@latest`, svg: npmSvg },
	{ name: 'bun', cli: `bun create ${packageName}@latest`, svg: bunSvg },
	{ name: 'yarn', cli: `yarn create ${packageName}@latest`, svg: yarnSvg }
];

export const groupIconMdPlugin = (md: string, options: GroupIconMdPluginOptions = {}) => {
	const codeGroupSvgData = options.codeGroupSvgData ?? createCodeGroupSvgData('stdf');
	...
	const codeGroupHtmlList = codeGroupList.map((item, index) => {
		...
		item.forEach((subItem, subIndex) => {
			const tabId = `${index}-${subIndex}`;
			const label = `
			<input id="tab-${tabId}" type="radio" name="tab-${index}" class="peer/tab-${tabId} w-0 hidden" ${subIndex === 0 ? 'checked' : ''} />
				<label
					for="tab-${tabId}"
					class="border-b-2 border-black/10 dark:border-white/10 peer-checked/tab-${tabId}:border-primary dark:peer-checked/tab-${tabId}:border-dark transition duration-300 cursor-pointer py-2 px-3 inline-flex items-center gap-1 text-xs"
				>
