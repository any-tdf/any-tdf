...
const baseUrl = process.env.VUE_CONFETTI_DOCS_VERIFY_BASE_URL || 'http://127.0.0.1:5563/?lang=en_US';
const debugPort = Number(process.env.VUE_CONFETTI_DOCS_BROWSER_DEBUG_PORT || 9234);
const chromePath =
...
const userDataDir = mkdtempSync(join(tmpdir(), 'vue-confetti-browser-'));
...
for (let i = 0; i < 100; i += 1) {
	const ready = await runInPage<boolean>("return document.readyState === 'complete' && !!document.querySelector('button');").catch(
		() => false
	);
	if (ready) break;
	await sleep(100);
...
if (initial.title !== '@any-tdf/vue-confetti') throw new Error(`Unexpected title: ${initial.title}`);
...
if (constantDemo.pieces !== 20 || constantDemo.iterationCount !== 'infinite')
	throw new Error('Constant demo did not render 20 infinite pieces.');
...
if (fullscreenDemo.top !== '-50px' || fullscreenDemo.left !== '0px')
	throw new Error('Fullscreen layer is not anchored like the upstream demo.');
