...
type ApiRichnessProps = {
	lang?: 'zh_CN' | 'en_US';
};

const ApiRichness = ({ lang = 'zh_CN' }: ApiRichnessProps) => {
	...
	return (
		<section className="p-6 md:p-8">
			<div className="mx-auto max-w-6xl">
				...
				{/* 数据统计 */}
				<div ref={countUpRef} className="mb-12 grid grid-cols-1 border-l border-t border-(--site-divider) sm:grid-cols-2">
					{stats.map((stat, i) => (
						<div className="border-b border-r border-(--site-divider) bg-transparent p-6 text-center" key={stat.label}>
							<div className="mx-auto mb-3 flex size-12 items-center justify-center bg-primary/10 dark:bg-dark/10">
								<svg className="size-6 text-primary dark:text-dark" viewBox="0 0 24 24" fill="currentColor">
									<path d={stat.icon} />
								</svg>
							</div>
							<div className="text-3xl md:text-4xl font-bold text-primary dark:text-dark mb-2">
								{animatedValues[i]}
								{stat.suffix}
							</div>
							<div className="text-base font-medium text-gray-900 dark:text-gray-100 mb-1">{stat.label}</div>
				...
				{/* 特性亮点 */}
				<div className="grid grid-cols-1 border-l border-t border-(--site-divider) md:grid-cols-2">
					{features.map((feature) => (
						<div className="flex gap-4 border-b border-r border-(--site-divider) bg-transparent p-6" key={feature.title}>
							<div className="bg-primary/10 dark:bg-dark/10 flex size-10 shrink-0 items-center justify-center">
								<svg className="size-5 text-primary dark:text-dark" viewBox="0 0 24 24" fill="currentColor">
									<path d={feature.icon} />
								</svg>
							</div>
							<div>
								<h3 className="text-base font-semibold text-gray-900 dark:text-gray-100 mb-1">{feature.title}</h3>
