...
const MobileAdvantages = ({ lang = 'zh_CN' }: MobileAdvantagesProps) => {
		...
		<section className="p-4 md:p-8">
			<div className="mx-auto max-w-6xl">
				...
				<div className="text-center mb-12">
					...
					<h2 className="mb-4 text-3xl font-bold text-(--site-text) md:text-4xl">{isZh ? '触手可及，流畅自然' : 'Touch & Feel Natural'}</h2>
				...
				{/* 特性网格 */}
				<div className="grid grid-cols-1 border-l border-t border-(--site-divider) md:grid-cols-2 lg:grid-cols-3">
					{mobileFeatures.map((feature) => (
						<div
							className="group border-b border-r border-(--site-divider) bg-transparent p-6 transition-colors hover:bg-primary/5 dark:hover:bg-dark/5"
							key={feature.title}
						>
							{/* 图标 */}
							<div className="bg-primary/10 dark:bg-dark/10 mb-4 flex size-14 items-center justify-center">
							...
							<p className="opacity-60 mb-4 text-sm leading-relaxed">{feature.desc}</p>

							{/* 详情列表 */}
							<div className="flex flex-wrap gap-2">
								{feature.details.map((detail) => (
									<span
										className="bg-primary/5 dark:bg-dark/10 text-primary dark:text-dark border border-primary/10 px-2 py-1 text-xs dark:border-dark/10"
										key={detail}
									>
										{detail}
									</span>
