...
import { BottomSheet, Calendar, Button } from 'rtdf/components';
					...
					<div className="mb-12 text-center">
						...
						<div className="mb-6 inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/5 px-4 py-1.5 text-sm text-primary dark:border-dark/20 dark:bg-dark/5 dark:text-dark">
							<svg className="size-4" viewBox="0 0 24 24" fill="currentColor">
								<path d="M7 4V20H17V4H7ZM6 2H18C18.5523 2 19 2.44772 19 3V21C19 21.5523 18.5523 22 18 22H6C5.44772 22 5 21.5523 5 21V3C5 2.44772 5.44772 3 6 2ZM12 17C12.5523 17 13 17.4477 13 18C13 18.5523 12.5523 19 12 19C11.4477 19 11 18.5523 11 18C11 17.4477 11.4477 17 12 17Z" />
							</svg>
					...
					<div className="mx-auto max-w-4xl">
						<div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
							{mobileFeatures.map((feature) => (
								<div
									className="group rounded-2xl border border-gray-200/50 bg-white/80 p-5 shadow-lg backdrop-blur-sm transition-all hover:shadow-xl hover:-translate-y-1 dark:border-white/10 dark:bg-gray-900/80"
									key={feature.title}
								>
									<div className="mb-4 flex size-12 items-center justify-center rounded-xl bg-linear-to-br from-primary to-primary-700 shadow-lg shadow-primary/25 dark:from-dark dark:to-dark-700 dark:shadow-dark/25">
						...
						{/* 交互演示区域 */}
						<div className="mt-8 rounded-2xl border border-gray-200/50 bg-white/60 p-6 shadow-lg backdrop-blur-sm dark:border-white/10 dark:bg-gray-900/60">
							<div className="mb-4 text-center">
								<div className="text-lg font-semibold text-gray-800 dark:text-white">
									{isZh ? '点击体验移动端组件' : 'Tap to Experience Mobile Components'}
								</div>
								<div className="mt-1 text-sm text-gray-500 dark:text-gray-400">
									{isZh ? '这些组件专为触屏交互设计，支持手势操作' : 'Components designed for touch interaction with gesture support'}
								</div>
							</div>

							{/* 已选日期显示 */}
							{selectedDate ? (
								<div className="mb-4 rounded-xl bg-primary/10 p-3 text-center dark:bg-dark/10">
									<div className="text-xs text-gray-500 dark:text-gray-400">{isZh ? '已选日期' : 'Selected Date'}</div>
									<div className="text-xl font-bold text-primary dark:text-dark">{selectedDate}</div>
								</div>
							) : null}

							{/* 操作按钮 */}
							<div className="flex flex-col gap-3 sm:flex-row sm:justify-center">
								<Button fill="base" onClick={() => setShowBottomSheet(true)}>
								...
								<Button fill="line" onClick={() => setShowCalendar(true)}>
									<svg className="mr-2 size-4" viewBox="0 0 24 24" fill="currentColor">
							...
							<div className="mt-5 flex flex-wrap justify-center gap-2">
								{['BottomSheet', 'Calendar', 'Swiper', 'Picker', 'ActionSheet', 'Toast'].map((comp) => (
									<span
			...
			>
				<div className="p-4">
					<div className="mb-4 text-sm text-gray-600 dark:text-gray-400">
						{isZh
							? '这是一个可以拖拽调整高度的底部弹窗组件，支持多个停靠高度。'
							: 'This is a draggable bottom sheet with multiple snap heights.'}
					</div>
					<div className="grid grid-cols-3 gap-3">
						{[1, 2, 3, 4, 5, 6].map((i) => (
							<div
								className="flex aspect-square items-center justify-center rounded-xl bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"
								key={i}
							>
								{i}
							</div>
					...
					<div className="mt-4">
						<Button fill="base" size="full" onClick={() => setShowBottomSheet(false)}>
							{isZh ? '关闭' : 'Close'}
						</Button>
					</div>
				</div>
