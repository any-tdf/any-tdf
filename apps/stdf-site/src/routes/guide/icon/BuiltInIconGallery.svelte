	...
	<table class="w-full min-w-max border-collapse text-left text-sm">
		<thead class="bg-gray-50 text-gray-700 dark:bg-gray-900 dark:text-gray-200">
			<tr>
				...
				{#each builtInIconLibraryList as library (library)}
					<th class="px-4 py-3 text-center font-semibold">{builtInIconLibraryLabelMap[library]}</th>
				{/each}
			</tr>
		</thead>
		<tbody class="divide-y divide-gray-200 dark:divide-gray-800">
			{#each iconRows as item (item.key)}
				<tr>
						...
						<td class="px-4 py-3">
							<div class="flex items-center justify-center text-gray-900 dark:text-gray-100">
								<svg
									...
									aria-hidden="true"
								>
									{#each item.icons[library].nodes as node, index (resolveSvgNodeKey(node, index))}
										{#if node.type === 'path'}
											<path
												d={node.d}
