		...
		<div class="grid grid-cols-1 border-l border-t border-(--site-divider) md:grid-cols-2 lg:grid-cols-3">
			{#each mobileFeatures as feature (feature)}
				...
				>
					<!-- 图标 -->
					<div
						class="bg-primary/10 dark:bg-dark/10 mb-4 flex size-14 items-center justify-center"
					>
					...
					<div class="flex flex-wrap gap-2">
						{#each feature.details as detail (detail)}
							<span class="bg-primary/5 dark:bg-dark/10 text-primary dark:text-dark border border-primary/10 px-2 py-1 text-xs dark:border-dark/10">
								{detail}
							</span>
						{/each}
					</div>
