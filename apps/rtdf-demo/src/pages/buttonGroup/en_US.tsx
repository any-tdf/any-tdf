...
import type { ButtonGroupItemProps, ButtonGroupProps } from 'rtdf/types';
import { ButtonGroup, Icon, Slider, Toast } from 'rtdf/components';
...
function ButtonGroupEn() {
	...
	const textOnlyItems = useMemo<ButtonGroupItemProps[]>(
		() => [{ text: 'Cancel' }, { text: 'Confirm', onClick: () => setVisible(true) }],
		[]
	);
	...
	const disabledItems = useMemo<ButtonGroupItemProps[]>(
		() => [{ text: 'Enabled' }, { text: 'Disabled', disabled: true }, { text: 'Enabled' }],
		[]
	);

	const textIconAndIconItems = useMemo<ButtonGroupItemProps[]>(
		() => [{ text: 'Search', icon: { name: 'ri-search-line', size: 18 } }, { icon: { name: 'ri-filter-line', size: 20 } }],
		[]
	);
			...
			<div>
				...
				<div className="px-4 pb-4">
					<Slider
						...
						onChange={(v: number) => setRadiusIndex(v)}
					/>
				</div>
