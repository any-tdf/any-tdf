...
<div class="min-h-screen bg-bg-base pb-10 text-text-primary dark:bg-bg-base-dark dark:text-text-dark">
	<div class="mx-4 mt-4 rounded-md bg-bg-surface p-4 text-sm dark:bg-bg-surface-dark">
		...
		<div class="mt-3">
			<Button size="auto" injClass="px-4" onclick={() => (failNext = true)}>下一次加载失败</Button>
		</div>
	</div>
	...
	<div class="mx-4 mt-8 text-lg font-bold">自定义状态内容</div>
	<div class="mx-4 mt-1 text-sm text-text-primary/60 dark:text-text-dark/60">snippet 的 detail 参数提供 status 和 retry，错误内容可直接绑定重试。</div>
	<div class="mx-4 mt-4 overflow-hidden rounded-md bg-bg-surface dark:bg-bg-surface-dark">
	...
	<InfiniteScroll bind:loading={customLoading} bind:finished={customFinished} bind:error={customError} onload={loadCustomMore}>
		...
		{#snippet finishedChild()}
			<div class="inline-flex min-h-12 items-center text-text-primary/40 dark:text-text-dark/40">— 已经到底啦 —</div>
		{/snippet}
		{#snippet errorChild(detail)}
			<div class="inline-flex min-h-12 items-center gap-3">
				<span class="text-error">网络异常，加载失败</span>
				<Button size="auto" injClass="px-4" onclick={detail.retry}>点击重试</Button>
			</div>
		{/snippet}
	</InfiniteScroll>

	<div class="mx-4 mt-8 text-lg font-bold">嵌套滚动容器</div>
	<div class="mx-4 mt-1 text-sm text-text-primary/60 dark:text-text-dark/60">通过 scrollTarget 指定滚动容器，容器内滚动到底部附近触发加载。</div>
	...
	<div class="mx-4 mt-8 text-lg font-bold">顶部加载</div>
	<div class="mx-4 mt-1 text-sm text-text-primary/60 dark:text-text-dark/60">direction 为 up 时，滚动到顶部附近加载更早的数据，适合聊天记录场景。</div>
	...
	<div class="mx-4 mt-8 text-lg font-bold">手动检测</div>
	<div class="mx-4 mt-1 text-sm text-text-primary/60 dark:text-text-dark/60">数据或容器尺寸变化后，可通过组件暴露的 check 方法主动检测。</div>
	<div class="mx-4 mt-3">
		<Button size="auto" injClass="px-4" onclick={removeAndCheck}>删除前 6 条并检测</Button>
	</div>
	<div class="mx-4 mt-4 overflow-hidden rounded-md bg-bg-surface dark:bg-bg-surface-dark">
