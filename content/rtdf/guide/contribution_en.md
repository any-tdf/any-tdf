...
Thank you for using and supporting RTDF.

RTDF is the React component library in the Any TDF monorepo. The repository also contains the shared foundation, STDF, VTDF, framework Demos and documentation sites, build tooling, the project generator, the VS Code extension, and offline AI Skills. Contributions to components, documentation, tests, tooling, examples, and translations are welcome.
...
- A bug report should identify the affected package and version, React and browser versions, runtime environment, minimal reproduction, actual result, and expected result. Include screenshots or a recording for visual issues when possible.
...
| `packages/rtdf`                                     | React rendering, events, composition, children, and hooks.                       |
| `packages/stdf`, `packages/vtdf`                    | Framework-native Svelte and Vue implementations.                                 |
| `apps/rtdf-demo`                                    | RTDF component Demos, interactions, SSR, and parity validation.                  |
| `apps/rtdf-site`                                    | The RTDF documentation site.                                                     |
| `apps/site-common`                                  | Shared code for all three sites and the component documentation sources.         |
| `content/rtdf/guide`                                | RTDF guide content, including this page.                                         |
| `content/rtdf/components`                           | Generated site content; do not edit it directly.                                 |
...
Framework-neutral behavior belongs in `packages/common`. React-specific DOM access, lifecycle, events, children, hooks, and rendering belong in `packages/rtdf`. When shared component behavior changes, inspect the STDF, RTDF, and VTDF implementations, Demos, documentation, and parity checks together.
...
`bun run dev:rtdf` starts the RTDF documentation site and Demo together:

- Documentation site: `http://localhost:5554`
- Demo: `http://localhost:8887`

Switch the browser developer tools to mobile mode when viewing the Demo. You can also use the [online RTDF Demo](https://stackblitz.com/github/any-tdf/any-tdf?startScript=dev%3Artdf).

## Change Code and Documentation

### Components and Demos

- Decide whether the change belongs in `packages/common` or `packages/rtdf`, and reuse an existing repository pattern when possible.
- Update the corresponding Demo and validation scripts in `apps/rtdf-demo` when observable component behavior changes.
...
- Component documentation sources live in `apps/site-common/docs/component-docs`. Put common content in `shared` and RTDF-only content in `targets/rtdf`.
- `content/rtdf/components` is generated output and must not be edited directly. After changing component documentation, run:
...
- Guide content lives in `content/rtdf/guide`. Keep the Chinese `.md` and English `_en.md` files in sync.
- Run `bun run generate:check` after changing a generator or its inputs, and inspect every generated diff.

### Changesets

Run `bun run changeset` from the repository root when a change affects the behavior, API, dependencies, or package contents of a public npm package. Write the Changeset summary in English and select packages by actual ownership: choose `@any-tdf/common` for shared behavior and `rtdf` for React-specific implementation. Site-only, Demo-only, test-only, and repository documentation changes usually do not need a Changeset.

## Validate Changes

Start with the checks closest to the changed scope. A typical RTDF component change should run at least:
