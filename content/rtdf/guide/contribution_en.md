...
After the development server starts, open `http://localhost:8887` and switch the browser developer tools to mobile mode.

The RTDF component source is in `packages/rtdf/src/lib/components`, and its Demo is in `apps/rtdf-demo`. The development server updates as you modify the source.
...
Use [RTDF Demo](https://stackblitz.com/github/any-tdf/any-tdf?startScript=dev%3Artdf) for online debugging if you do not want to set up the repository locally.
...
- (Optional) Update the Chinese and English documentation in `content/rtdf/components`. Depending on the change, you may need to update the API, FAQ, guide, or version documents. Add a tag for version changes; see the [RTDF version tags](https://github.com/any-tdf/any-tdf/blob/main/content/rtdf/components/button/version.md?plain=1).
- If the change affects a published package, run `bun run changeset` to add a Changeset.
- Install dependencies with Bun at the repository root and verify the Demo with `bun run dev:rtdf`.
