...
STDF 使用 Tailwind CSS 4 的 `@plugin` 语法定义主题。主题插件由 `stdf/theme` 提供，在项目入口 CSS 文件中使用 `@plugin "stdf/theme"` 语法定义主题。

`switchTheme`、`switchMode`、`getTheme`、`getMode` 等运行时方法仍然从 `stdf/theme` 导入。
