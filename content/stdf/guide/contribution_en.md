...
After the development server starts, open `http://localhost:8888` and switch the browser developer tools to mobile mode.

The STDF component source is in `packages/stdf/src/lib/components`, and its Demo is in `apps/stdf-demo`. The development server updates as you modify the source.
...
Use [STDF Demo](https://stackblitz.com/github/any-tdf/any-tdf?startScript=dev%3Astdf) for online debugging if you do not want to set up the repository locally.
...
- (Optional) Update the Chinese and English documentation in `content/stdf/components`. Depending on the change, you may need to update the API, FAQ, guide, or version documents. Add a tag for version changes; see the [STDF version tags](https://github.com/any-tdf/any-tdf/blob/main/content/stdf/components/button/version.md?plain=1).
- If the change affects a published package, run `bun run changeset` to add a Changeset.
- Install dependencies with Bun at the repository root and verify the Demo with `bun run dev:stdf`.
