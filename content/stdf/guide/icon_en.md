...
Some STDF components use built-in SVG icons internally, such as `Alert`, `Toast`, `Checkbox`, `Radio`, and `Pagination`. The default built-in icon library is `remix`. You can also switch to `lucide`, `phosphor`, `tabler`, `iconoir`, or `reicon`. To switch those internal component icons, set `builtInIconLibrary` on `ConfigProvider`.
