...
默认头部会显示下拉、释放、刷新中和成功文案。也可以通过 `normalChild`、`pullingChild`、`canReleaseChild`、`refreshingChild`、`successChild` snippet 自定义各状态内容，snippet 的 `detail` 参数提供 `status`、`distance` 和 `progress`，可用于驱动进度环、箭头旋转等动画。
