...
Input 组件内部监听了键盘按键事件，当按下任意按键时会触发 `@keydown` 事件，返回的 `value` 表示按下的按键 key（备注：`keyCode` 已弃用，参考 [MDN](https://developer.mozilla.org/zh-CN/docs/Web/API/KeyboardEvent/keyCode)），可根据此值做一些特殊处理。参考示例。
...
当然，如果这些配置项不能满足需求，它们也都支持使用 Vue Slot 渲染，请参考示例。
