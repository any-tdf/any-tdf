...
私以为，分页不应该是上滑加载下一页，这种交互会让用户无心理预期而产生不适，所以 VTDF 的分页组件交互默认满足以下几个条件：
...
Pagination 组件对外暴露了三个事件：`@change`、`@next`、`@pre`，分别对应页码变化、下一页、上一页。`@next` 和 `@pre` 事件都会同时触发 `@change` 事件，请按需监听。
