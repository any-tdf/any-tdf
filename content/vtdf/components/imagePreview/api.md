...
| 名称   | 类型                                        | 参数               | 描述             |
| ------ | ------------------------------------------- | ------------------ | ---------------- |
| change | `(index: number) => void`                   | index: 当前索引    | 切换图片时触发。 |
| close  | `() => void`                                | -                  | 关闭时触发。     |
| scale  | `(scale: number) => void`                   | scale: 缩放比例    | 缩放时触发。     |
| rotate | `(rotation: 0 \| 90 \| 180 \| 270) => void` | rotation: 旋转角度 | 旋转时触发。     |

## ImagePreview Children

| 名称         | 类型     | 参数           | 说明             |
| ------------ | -------- | -------------- | ---------------- |
| children     | Vue Slot | item, index    | 自定义内容。     |
| loadingChild | Vue Slot | -              | 自定义加载中。   |
| errorChild   | Vue Slot | -              | 自定义加载失败。 |
| indexChild   | Vue Slot | current, total | 自定义索引显示。 |
