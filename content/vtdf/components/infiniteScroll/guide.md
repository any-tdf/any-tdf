...
- `direction` 为 `down` 时，距离底部小于等于 `offset` 会触发 `load`。
- `direction` 为 `up` 时，距离顶部小于等于 `offset` 会触发 `load`。
...
- `error` 状态内容点击后，会以 `isRetry = true` 重新触发 `load`。
...
默认会展示加载中、加载完成和加载失败文案。也可以使用 `loadingChild`、`finishedChild`、`errorChild` 具名 slot 自定义状态内容，slot 的 `detail` 参数提供 `status` 和 `retry`，其中 `retry` 可直接绑定到自定义重试按钮。传入默认 slot 则完全接管状态内容。
