...
| triggerRef      | `HTMLElement\|Vue Ref<HTMLElement\|null>\|null`    | `null`                            | N    | 触发元素引用，支持 DOM 元素或 Vue ref 对象。                       |
...
| close        | `() => void`                              | -                                             | 关闭时触发。                                 |
| cancel       | `() => void`                              | -                                             | 点击取消时触发。                             |
| click-action | `(index: number, action: Action) => void` | index - 点击索引值<br />action - 点击选项数据 | 点击选项时触发，即使点击选项不关闭也会触发。 |
