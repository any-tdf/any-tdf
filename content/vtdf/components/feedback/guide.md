...
函数式反馈 API 允许你在任意位置（包括非 Vue 组件代码）通过函数调用使用 Toast、Dialog、Modal、Alert、Loading 这 5 种反馈组件。
...
| 特性       | 函数式 API      | 组件式                                         |
| ---------- | --------------- | ---------------------------------------------- |
| 使用位置   | 任意 JS/TS 代码 | 仅 Vue 组件 Vue SFC                            |
| 自定义内容 | 仅支持文本      | 支持 Vue Slot                                  |
| 受控状态   | 不支持          | 支持受控 props（例如 `visible` 配合 `@close`） |
| 状态管理   | 自动管理        | 手动管理                                       |
| 复杂交互   | 有限            | 完全支持                                       |
...
函数式 API 默认不依赖 Vue Context，如果需要手动设置语言，可以使用：
...
<template>
	...
	<button @click="showCustomToast = true">显示自定义 Toast</button>
</template>
```
