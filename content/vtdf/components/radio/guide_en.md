...
Radio accepts a `value` prop as the initial identifier for internal Radio items. When a RadioItem is clicked, Radio gets the current value through the `@change` event, which is the `name` of the currently selected RadioItem.
...
When `radioChild` is passed, the content area of each RadioItem in Radio uses the content area of `radioChild`. You can freely design selected and unselected effects with the value obtained from Radio's `@change` event.
