...
VTDF 提供符合开放 Agent Skills 格式的 AI Skill，帮助编码代理使用准确的 VTDF 组件 API、Vue 3 用法、Tailwind CSS 4 主题、图标、国际化和脚手架约定。Skill 会按需加载离线资料，普通 VTDF 项目不需要同时克隆整个 Monorepo。

这个 Skill 是 Agent 知识包，不是应用运行时依赖。不要执行 `bun add vtdf-skill`。
...
- Skill 目录：`packages/skills/vtdf-skill/vtdf`
- 格式：`SKILL.md`、`references/`、`scripts/`、`data/` 和可选的 `agents/openai.yaml`

目录名与 `SKILL.md` 中的 `name: vtdf` 保持一致，可被兼容 Agent Skills 的客户端直接识别。
...
安装器会从 GitHub 子目录下载完整 Skill。安装完成后，在下一轮任务中使用 `$vtdf`；如果客户端没有立即显示新 Skill，请重新加载或重启客户端。
...
用户级共享可以复制到 `~/.agents/skills/`。如果客户端使用其他搜索目录，也应复制完整的 `vtdf` 目录，不要只复制 `SKILL.md`，否则组件资料和主题脚本无法使用。
...
```text
$vtdf 使用 VTDF 的 Button、Toast 和 Form 编写一个 Vue 3 登录页，并核对每个组件的 API。
```
...
Skill 会先确认项目中的 VTDF 版本，再读取任务所需资料。涉及组件时必须读取对应详情文件，不应根据组件名称猜测 Props、Emit、Slot、公开方法或 Composable。
...
```text
$vtdf 使用 ANYTDF 预设生成完整主题配置。
```

生成结果可包含 `@plugin "vtdf/theme/plugin"`、`@theme` 或 JSON。
