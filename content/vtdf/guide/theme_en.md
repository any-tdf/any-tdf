...
VTDF provides the mode switching method in the NPM package, which essentially switches by modifying the `data-mode` attribute of the `html` tag.
...
VTDF supports switching between multiple themes, including [color system](/guide/color), border radius, background color, text color, and other configurations. The essential principle is to set some attributes as CSS variables and switch themes by modifying the `data-theme` attribute.

### Configuration

VTDF uses Tailwind CSS 4's `@plugin` syntax to define themes. Use the `@plugin "vtdf/theme"` syntax in the project entry CSS file to define themes.

#### Using Built-in Themes

VTDF comes with 42 carefully designed themes, just pass the theme name to use:
...
If your project **doesn't need to switch themes**, you only need to configure CSS variables in `@theme`, without using `@plugin "vtdf/theme"`:
...
> **Note**: Theme variables generated by `@plugin "vtdf/theme"` will override the values in `@theme` through `[data-theme="xxx"]` selectors.
...
VTDF comes with 42 carefully designed themes that can be used directly:

| Theme        | Description             | Theme      | Description             |
| ------------ | ----------------------- | ---------- | ----------------------- |
| VTDF         | Default VTDF theme      | Nintendo   | Red & Blue Classic      |
...
Use the `switchMode` method from the VTDF NPM package, passing `'primary'` or `'dark'` to switch between light and dark modes.
...
2. Define switchable themes using `@plugin "vtdf/theme"`:
