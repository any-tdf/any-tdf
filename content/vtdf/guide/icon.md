> 从 v1.1.0 开始，VTDF 支持 [Iconify](https://iconify.design) 方式使用图标。

## SVG Sprites

VTDF 图标组件的 `type` 默认为 `symbol`，即使用 SVG Sprites 技术使用图标，这样可以减少 HTTP 请求，提高页面性能。

VTDF 的 SVG Sprites 使用 [SVG symbol](https://developer.mozilla.org/en-US/docs/Web/SVG/Element/symbol)，大致原理类似于 CSS 中的 Sprite 技术，将项目中的 SVG 拼接在一个文件内，通过 SVG 中的 use 元素找到对应的图标来显示。
...
VTDF 使用 [@any-tdf/vite-plugin-svg-symbol](https://www.npmjs.com/package/@any-tdf/vite-plugin-svg-symbol) 将项目中的 SVG 文件合并成 SVG Sprites。由于 SVG 格式的多样性，可能存在处理不准确的情况，请到 [GitHub](https://github.com/any-tdf/any-tdf/issues) 提 Issue，注明 SVG 文件的具体情况。
...
VTDF 也可以通过设置 `type` 为 `iconify` 或 `iconify-color` 使用 [Iconify](https://iconify.design) 方式使用图标。

Iconify 有很多种使用方式，VTDF 选择的是 Tailwind 4 插件的方式，这样对代码的侵入性最小，可参考 [Iconify for Tailwind CSS](https://iconify.design/docs/usage/css/tailwind/tailwind4)。在 VTDF 中使用方式如下：
...
VTDF 的部分组件内部会使用内置 SVG，例如 `Alert`、`Toast`、`Checkbox`、`Radio`、`Pagination` 等。默认内置图标库是 `remix`，也可以切换为 `lucide`、`phosphor`、`tabler`、`iconoir` 或 `reicon`。如果希望这些组件内部图标切换为其他内置图标库，可以在 `ConfigProvider` 中设置 `builtInIconLibrary`。
...
一般来说，在 VTDF 项目中可以有以下几种使用 SVG 图标方式。
